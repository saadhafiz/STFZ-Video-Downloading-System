...
$.ajax({
 ...
 // Define 
        ...
        if (!$.trim(data)){ 
              alert("1 - Unable To Get Download Link Please Check URL and Contact us on Social Media @Oay_Saad");
              document.getElementById("loading").style = "display:none";
         }else {
         document.getElementById("loading").style = "display:none";
...
if (match && match[2].length == 11) {
  ...
  VTHUMB = vidThumb;
         ...
         document.title  = "Download "+vidTitle+" STFZDownloader";
         }
         if(vidDescription)
         {
...
downloadV.innerHTML += "<a href='"+obj.medias[i].url+"'><button style='background:"+bgcol+"' class='dlbtns'>"+obj.medias[i].quality + myParam+"</button></a>";
         ...
         { 
         alert("2 -Server Down due to Too Many Requests please Contact us on Social Media @Oay_Saad");
